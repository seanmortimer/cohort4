...
test('adding and deleting for stack', () => {
  ...
  expect(lifo.head.show()).toEqual(['Bat', 20]);
  expect(lifo.currentNode.show()).toEqual(['Bat', 20]);
  expect(lifo.tail.show()).toEqual(['Ant', 10]);

  lifo.addToStack('Cat', 30);
  expect(lifo.size).toBe(3);
  expect(lifo.head.show()).toEqual(['Cat', 30]);
  expect(lifo.currentNode.show()).toEqual(['Cat', 30]);
  expect(lifo.tail.show()).toEqual(['Ant', 10]);
...
test('adding and deleting for queue', () => {
  ...
  expect(fifo.currentNode.show()).toEqual(['Bat', 20]);

  fifo.enqueue('Cat', 30);
  expect(fifo.size).toBe(3);
  expect(fifo.head.show()).toEqual(['Ant', 10]);
  expect(fifo.tail.show()).toEqual(['Cat', 30]);
  expect(fifo.currentNode.show()).toEqual(['Cat', 30]);
